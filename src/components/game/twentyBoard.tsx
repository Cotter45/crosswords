...
function TwentyBoard({
  ...
  useEffect(() => {
    const { board, definitions, words, indexes, letterCount } =
      createTenBoard(20);
    setWords(words);
    setIndexes(indexes);
    setGrid(board);
    ...
    <div className="twenty-board">
      {grid &&
        grid.map((row, rowIndex) => (
          <div key={rowIndex} className="twenty-board-row">
            {row.map((cell, cellIndex) => (
              <div
                key={cellIndex + 1}
                style={{ position: "relative", width: "1fr" }}
              >
                <input
                  maxLength={1}
                  disabled={!cell}
